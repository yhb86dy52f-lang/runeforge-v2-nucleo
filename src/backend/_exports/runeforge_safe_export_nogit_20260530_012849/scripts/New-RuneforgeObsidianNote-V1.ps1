...
function ConvertTo-SafeTag {
    param([string]$Text)

    $tag=$Text -replace '\s+', '_'
...
$cleanTags=@()
foreach($tag in $Tags){
    $t=ConvertTo-SafeTag -Text $tag
    if(-not [string]::IsNullOrWhiteSpace($t)){
        $cleanTags += $t
    }
}

$tagInline=($cleanTags | ForEach-Object { "#"+$_ }) -join " "
...
$lines.Add("source: "+$Source.Replace('"',"'")) | Out-Null
$lines.Add("status: active") | Out-Null
$lines.Add("---") | Out-Null
$lines.Add("") | Out-Null
$lines.Add("# "+$Title) | Out-Null
$lines.Add("") | Out-Null
$lines.Add("Tags: "+$tagInline) | Out-Null
$lines.Add("") | Out-Null
...
$trace=[pscustomobject]@{
    ...
    root=$Root
    backend="NO_TOCADO"
    runeforge="OBSIDIAN_BRIDGE_V1_2"
}
