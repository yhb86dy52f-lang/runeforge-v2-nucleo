...
; =====================================================================
; RUNEFORGE V43.6.1 AHK CALLBACK FIX
; Mini panel + restart callbacks
; =====================================================================

LaunchMini(*) {
    global RF

    launcher := RF["ROOT"] . "\scripts\Start-Runeforge-Panel.ps1"

    if !FileExist(launcher) {
        MsgBox("No existe launcher:`n" . launcher, "Runeforge", "Iconx")
        return
    }

    cmd := "pwsh.exe -NoLogo -ExecutionPolicy Bypass -File " . Q(launcher) . " -Mode Quick"
    Run(cmd, RF["ROOT"])

    try Trace("LAUNCH_MINI", "V43.6.1")
    try Toast("Mini panel abierto")
}
...
RestartPanel(*) {
    ...
    cmd := "pwsh.exe -NoLogo -ExecutionPolicy Bypass -File " . Q(launcher) . " -Mode Full"
    Run(cmd, RF["ROOT"])

    try Trace("RESTART_PANEL", "V43.6.1")
    ExitApp()
}
...
Main.Add("Button", "x590 y616 w86 h30", "🧊 Mini").OnEvent("Click", (*) => LaunchMini())
Main.Add("Button", "x684 y616 w116 h30", "🔄 Reiniciar").OnEvent("Click", (*) => RestartPanel())
Main.Add("Button", "x808 y616 w90 h30", "✖ Cerrar").OnEvent("Click", (*) => ExitApp())
